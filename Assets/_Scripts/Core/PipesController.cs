...
namespace AjaxNguyen.Core
{
    public class PipesController : MonoBehaviour
    {
        ...
        public void SetupPipe(float gapSize, float gapYPos, bool isVeticalMove, float ySpeed = 0f, float minHeight = 0f, float maxHeight = 0f) // Hàm để căn chỉnh lại cặp pipe
        {
            if (isVeticalMove)
            {
            ...
            transform.position = new Vector3(transform.position.x, gapYPos, -1);
